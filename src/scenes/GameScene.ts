...
export class GameScene extends Phaser.Scene {
  ...
  private setupHairStrands() {
    ...
    const count = Math.floor(5 * this.levelConfig.hairVolume);
    const positions = [
    ...
    for (let i = 0; i < Math.min(count, 8); i++) {
      ...
      const strand = this.add.image(pos.x + offsetX, pos.y + 40, strandKey)
        .setAlpha(0.7)
        .setDepth(1);
  ...
  private setupCurlInterference() {
    if (!this.levelConfig.hasCurlInterference) return;

    for (let i = 0; i < 4; i++) {
      const curl = this.add.image(
        this.headX + Phaser.Math.Between(-80, 80),
        this.headY + Phaser.Math.Between(-20, 80),
        'curl'
  ...
  private setupUI() {
    ...
    this.uiElements.push(
      this.add.text(15, 12, `关卡: ${this.levelConfig.name}`, {
        fontSize: '15px',
        fontFamily: 'system-ui',
        color: '#ffb6c1',
        fontStyle: 'bold',
  ...
  private updateUI() {
    ...
    const phaseNames: Record<GamePhase, string> = {
      [GamePhase.PARTITION]: '📋 分区',
      [GamePhase.GRAB]: '✋ 抓取',
      [GamePhase.CROSS]: '🔄 交叉',
      [GamePhase.TIGHTEN]: '💪 收紧',
  ...
  private updateCompletionBar() {
    ...
    const totalSteps = this.levelConfig.braidSteps.length;
  ...
  private updateHintAndStep() {
    ...
    if (this.currentStepIndex < this.levelConfig.braidSteps.length) {
      ...
      if (hintText) {
        hintText.setText(step.description);
      }
      if (stepText) {
        stepText.setText(
          `步骤 ${this.currentStepIndex + 1}/${this.levelConfig.braidSteps.length} | ` +
  ...
  private startPhase(phase: GamePhase) {
    ...
    switch (phase) {
      ...
      case GamePhase.GRAB:
        this.startGrabPhase();
        break;
      case GamePhase.CROSS:
        this.startCrossPhase();
        break;
      case GamePhase.TIGHTEN:
        this.startTightenPhase();
        break;
  ...
  private startPartitionPhase() {
    ...
    this.zoneAreas.forEach((za) => {
      ...
      const zoneColors: Record<HairZone, number> = {
        [HairZone.LEFT]: COLORS.zoneLeft,
        [HairZone.RIGHT]: COLORS.zoneRight,
        [HairZone.TOP]: COLORS.zoneTop,
        [HairZone.BACK]: COLORS.zoneBack,
      };
    ...
    const hintText = this.children.getByName('hintText') as Phaser.GameObjects.Text;
    if (hintText) {
      hintText.setText(`👆 点击闪烁的【${ZONE_NAMES[targetZone]}】进行分区`);
    }

    this.time.delayedCall(500, () => {
      this.enableZoneInteraction(targetZone);
    });
  }
  ...
  private enableZoneInteraction(targetZone: HairZone) {
    this.zoneAreas.forEach((za) => {
      const hitRect = this.add.rectangle(
        za.x + za.w / 2, za.y + za.h / 2,
        za.w, za.h, 0x000000, 0,
  ...
  private startGrabPhase() {
    ...
    const hintText = this.children.getByName('hintText') as Phaser.GameObjects.Text;
    if (hintText) {
      hintText.setText(`点击头发抓取${ZONE_NAMES[step.zone]}的发束`);
    }
    ...
    this.hairStrands.forEach((strand, i) => {
      ...
      strand.on('pointerover', () => {
        strand.setTint(0xff9999);
        strand.setScale(1.1);
      });
      strand.on('pointerout', () => {
        strand.setTint(0xffdddd);
        strand.setScale(1);
      });
      strand.on('pointerdown', () => {
        this.grabStrand(strand, i);
      });
    });
  }

  private grabStrand(strand: Phaser.GameObjects.Image, _index: number) {
    this.tweens.add({
      targets: strand,
      scaleX: 0.6,
      scaleY: 1.3,
  ...
  private startCrossPhase() {
    ...
    if (hintText) {
      hintText.setText(`🎵 节奏编发：音符到达判定线时按对应方向键！← 左 | → 右`);
    }
  ...
  private updateRhythmTrack(delta: number) {
    ...
    for (const note of this.rhythmNotes) {
      ...
      if (note.x < this.rhythmHitZoneX - 60 && !note.hit) {
        note.missed = true;
        if (note.sprite) {
          note.sprite.setTexture('rhythm-miss');
          note.sprite.setAlpha(0.6);
        }
        this.combo = 0;
        this.totalNotes++;
  ...
  private handleRhythmHit(direction: 'L' | 'R') {
    ...
    if (closestNote.sprite) {
      ...
      this.tweens.add({
        targets: (closestNote.sprite as any).dirLabel,
        alpha: 0,
        scaleX: 1.5,
        scaleY: 1.5,
        duration: 300,
  ...
  private startTightenPhase() {
    ...
    const hintText = this.children.getByName('hintText') as Phaser.GameObjects.Text;
    if (hintText) {
      hintText.setText('💪 最后一步：掌握力度，在最佳区域收紧辫子！');
    }
  ...
  private drawTightenBar() {
    ...
    if (!label) {
      const lbl = this.add.text(GAME_WIDTH / 2, barY - 22, '💪 空格键收紧辫子！在绿色区域按下', {
        fontSize: '14px',
        fontFamily: 'system-ui',
        color: '#ffb6c1',
        fontStyle: 'bold',
  ...
  private updateTightenBar(delta: number) {
    ...
    if (this.tightenPower >= 1) {
      this.tightenPower = 1;
      this.tightenDirection = -1;
    } else if (this.tightenPower <= 0) {
      this.tightenPower = 0;
      this.tightenDirection = 1;
    }
  ...
  private loosenBraidVisual() {
    this.braidSegments.forEach((seg) => {
      this.tweens.add({
        targets: seg,
        y: seg.y + Phaser.Math.Between(2, 5),
        alpha: 0.6,
        duration: 300,
      });
    });
  ...
  private completeStep() {
    ...
    if (this.currentStepIndex >= this.levelConfig.braidSteps.length) {
      ...
      for (let i = 0; i < 3; i++) {
        this.time.delayedCall(i * 100, () => {
          this.addParticles(
            ...
            this.headY + Phaser.Math.Between(-40, 40),
            'particle-gold',
          );
        });
      }
      ...
      const hintText = this.children.getByName('hintText') as Phaser.GameObjects.Text;
      if (hintText) {
        hintText.setText('✨ 太棒了！步骤完成！准备下一步...');
      }
